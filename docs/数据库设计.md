...
Phase 3 已由 Migration `0003_full_market_features` 落地。`security_features` 将收益、位置、均线、ATR、波动率、成交额和量比等高频筛选字段保存为类型化列，扩展状态保存于 JSONB；每行绑定 DAY/QFQ `series_revision_id` 和可选 `factor_revision_id`。Run、Feature、Regime 均禁止 UPDATE/DELETE，发布采用单事务 append-only 写入。
