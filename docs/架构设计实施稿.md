...
```text
EXPECTED → PENDING_IMPORT → COMPLETED
         ↘ MISSED / CANCELLED
```

Expected Run 仅表示“按 Profile 预期 ChatGPT 分析任务应当发生”，不代表服务器已经执行了 AI，也不代表 ChatGPT 已经运行。只有 AI Result 成功导入才进入 `COMPLETED`。用户可以标记“ChatGPT 已执行、待导入”；超出宽限期仍无结果则保留 `MISSED`，不能静默形成历史空洞。正式 A 股交易日历目前若未有经验证来源，应标记为 `UNKNOWN`，实施时补充可靠 Provider。
...
6. 返回导入预览；
7. 用户一次确认；
8. 按逻辑 Atomic Group 分别以事务写入 Market Review、Watchlist、Decision、Entry Plan、Review、Position Review 和 Evidence 关联；
9. 成功组对应的 Task Run 标记完成，失败组保持待修正状态；
...
source, source_reference, confidence, decision_id, entry_plan_id,
entry_plan_matched, execution_type, execution_deviation_pct,
execution_note, created_at
```

Ledger 不允许 UPDATE/DELETE。错误通过冲正或 Correction 追加。持仓必须由 Ledger 重建；Projection 只是可丢弃缓存。默认禁止超卖，除非账户明确允许相应业务。公司行动使用独立 Adjustment Ledger；当前可靠公司行动来源若未确认，标记 `UNKNOWN`。
...
价格偏差以计划价格区间为基准：区间内为匹配；区间外按与最近边界的差异计算，并保存当时计划版本。`time_deviation` 比较计划窗口与真实成交时间，`quantity_deviation` 比较计划数量/仓位与真实成交，`trigger_matched` 和 `cancel_condition_violated` 保留触发与取消条件执行事实。用户追高、延迟、减少数量、违反取消条件或自主交易不能污染 AI Entry Plan 本身的评价。
...
返回：真实数量、成本计算方法、平均成本、已实现/未实现收益、最新价格、逐笔交易、首次买入时间、持有交易日、原始 Decision、Entry Plan、最新 Review、止损/目标、支撑/阻力、5m/15m/60m/日/周结构、行业、基本面、Evidence、风险、时间效率和数据质量。
...
Performance 必须基于 Execution Deviation 区分“AI 计划错误”和“用户未按计划执行”。AI 选股与买点正确、但用户追高导致亏损时，Selection/Entry Plan 可保持正确，偏差归入 User Execution；AI 未建议买入而用户自主买入时记录 `MANUAL_TRADE_WITHOUT_AI_ENTRY`，不计入 AI Entry 绩效。
...
`agent_tasks`、`task_profiles`、`expected_runs`、`task_runs`、`ai_result_imports`、`ai_result_bundles`、`ai_result_atomic_groups`、`ai_result_envelopes`、`ai_result_dependencies`。

### 24.5 决策状态

`watchlists`、`watchlist_events`、`watchlist_proposals`、`decisions`、`decision_corrections`、`entry_plans`、`reviews`、`market_reviews`。

### 24.6 交易与持仓

`accounts`、`trade_ledger`、`trade_corrections`、`trade_import_batches`、`trade_drafts`、`image_imports`、`opening_positions`、`reconciliations`、`position_snapshot_drafts`、`position_projections`、`portfolio_snapshots`、`portfolio_preferences`、`corporate_action_adjustments`。Trade Ledger 应保存 Entry Plan 匹配状态和 Execution Deviation。
...
Single Envelope、AIResultBundle、整包预览、一次确认、按逻辑 Atomic Group 分组原子提交、Append-only MarketReview、Watchlist、Immutable Decision、Entry Plan、Append-only Review。

### Phase 8：Trade Ledger、截图、Opening Position 与 Portfolio

手工成交、图片 Draft、确认、Opening Position、Reconciliation、Immutable Ledger、Execution Deviation、Position Projection、Portfolio 事实与 Position Context。

### Phase 9：Action、Entry、Position Review 与时间效率

Raw/Action/Entry 分层、expected horizon、持仓成本相关 Review；结构化事实供 AI 判断，禁止统一固定 Final Score。
...
| 7 | Bundle 一次预览/确认；同股依赖同组原子；失败组不影响合法组；返回组级结果；MarketReview 可追溯 |
| 8 | 只有确认 Trade 才形成 Holding；OCR 只生成 Draft；Opening 不伪造 BUY；Ledger 可重建；偏差可追踪 |
| 9 | Action 显式考虑时间效率；Position Review 使用真实成本且不自动成交；Portfolio 不硬删机会 |
| 10 | 七类 Performance 分离；AI 计划与用户执行分开；Replay 无前视；Recall Miss 窗口成熟后生成 |
