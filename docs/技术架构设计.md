...
| V3 正式数据库 | PostgreSQL 17 + SQLAlchemy async + asyncpg | 已实现，生产待最终启用 |
| Migration | Alembic async | 已实现 |
| V3 Worker/Scheduler | 独立 Compose Profile + PostgreSQL advisory lock | Phase 2 已实现 |
...
包含 Evidence、Task、Decision、版本化 EntryPlan、Watchlist、PositionReview、Trade Ledger、Portfolio Adjustment、Portfolio、Performance 等业务不变量。不得依赖 FastAPI、httpx、模型 SDK或具体数据库驱动。
...
→ Successful/Failed Group Result → Task Group Counters/Audit Update
```

同组严格原子，不同组容错隔离。Confirm 绑定 Bundle Hash、Preview Revision 和 Idempotency Key。Task Run 根据 expected/successful/failed/pending Group Count 计算 `PENDING_IMPORT/PARTIAL_COMPLETED/COMPLETED`，而不是把部分成功误标为完成。

### 6.4 Trade Pipeline

```text
Manual/Image/Corporate Action → Draft or Pending Reconciliation → User Confirm
→ Immutable Ledger / Opening / Portfolio Adjustment / Reconciliation
→ Projection Rebuild → Portfolio Context
```

OCR、AI 和外部 Agent 都不能绕过 Draft 与用户确认。市场层 Corporate Action 不能直接修改账户持仓，必须转换成账户级 Append-only Portfolio Adjustment；来源不可靠时保持 UNKNOWN/Pending。
...
- EntryPlan 通过版本链和 Current Projection 表达当前有效计划，禁止 UPDATE 历史版本；
- Trade/Execution Deviation 永久绑定成交当时的 EntryPlan Version；
...
- Context 大小与裁剪；
- Bundle 成功/失败组数；
- Task Run 的 Expected/Successful/Failed/Pending Group Count 与 Partial Completed 数；
- PositionReview 创建数、无 Decision 的手工持仓 Review 数和建议动作分布；
- Portfolio Adjustment Pending/Confirmed、来源 UNKNOWN 和 Projection Rebuild 差异；
- EntryPlan Version 数、当前版本和成交绑定版本缺失数；
