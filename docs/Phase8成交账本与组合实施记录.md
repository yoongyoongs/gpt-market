...
2026-09-01 在隔离 PostgreSQL 17 完成并发超卖验收：100 股持仓并发确认两笔 80 股 SELL，只有一笔成功，另一笔在 Projection 行锁后明确拒绝，最终数量为 20；Draft/计划绑定、Projection Hash 和不可变迁移链回归通过。OCR 外部识别准确率不在本轮技术门禁内；生产未部署。
