...
V3 按 [架构设计实施稿](docs/架构设计实施稿.md) 分阶段建设。Phase 1 正式状态基础和 Phase 2 全市场历史行情底座已完成技术验收；Phase 3 的版本化 Feature Run、白名单 Universe Query 和事实型 Market Regime 已完成核心实现与 PostgreSQL 集成验收，正在执行 5,551 只全市场性能验收。生产 V3 仍默认关闭；详见 [工作状态](docs/工作状态.md)、[Phase 2 验收记录](docs/Phase2行情底座实施记录.md) 与 [Phase 3 实施记录](docs/Phase3全市场特征实施记录.md)。
