...
class ConfirmAIResultImportService:
    ...
    async def execute(
        self, import_id: UUID, command: AIResultConfirmCommand,
    ) -> AIResultConfirmResult:
        async with self._uow_factory() as uow:
            preview = AIResultImportPreview.model_validate(
                await uow.ai_imports.get_preview_payload(import_id)
            )
            await uow.ai_imports.claim_confirmation(
                import_id, command.preview_revision, command.bundle_hash,
                command.idempotency_key, command.confirmed_by,
            )
            await uow.commit()
        ...
        for group in preview.bundle.atomic_groups:
            preview_group = next(item for item in preview.groups if item.group_id == group.group_id)
            if not preview_group.valid:
                result = ConfirmedGroup(
                    group_id=group.group_id, status=GroupCommitStatus.FAILED,
                    error="; ".join(preview_group.errors), retryable=False,
                )
                failed.append(result)
                async with self._uow_factory() as uow:
                    await uow.ai_imports.fail_group(
                        preview.bundle.bundle_id, group.group_id, result.error or "invalid group"
                    )
                    await uow.commit()
                continue
            try:
                async with self._uow_factory() as uow:
                    ...
                    await uow.commit()
                successful.append(ConfirmedGroup(
                    group_id=group.group_id, status=GroupCommitStatus.COMMITTED,
                    result_ids=tuple(item.result_id for item in group.results),
        ...
        for task_run_id in preview.bundle.task_run_ids:
            async with self._uow_factory() as uow:
                task_statuses[task_run_id] = await uow.ai_imports.refresh_task_run(task_run_id)
                await uow.commit()
        async with self._uow_factory() as uow:
        ...
        return AIResultConfirmResult(
            ...
            successful_groups=tuple(successful), failed_groups=tuple(failed),
